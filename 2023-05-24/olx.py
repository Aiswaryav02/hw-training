...
def property(url, count,items):
    ...
    with sync_playwright() as pw:
            ...
            if desc_element is not None : descriptions = desc_element.text_content() ; descriptions = None

            price_element = page.query_selector('xpath=//span[@data-aut-id="itemPrice"]')
            if price_element is not None : price = price_element.text_content() ; price = None
           
            title_element = page.query_selector('xpath=//h1[@data-aut-id="itemTitle"]')
            if title_element is not None : property_title = title_element.text_content() ; property_title = None
            
            bathrooms_element = page.query_selector('xpath=//span[@data-aut-id="value_bathrooms"]')
            if bathrooms_element is not None : bathrooms = bathrooms_element.text_content() ; bathrooms = None
                
            bedrooms_element = page.query_selector('xpath=//span[@data-aut-id="value_rooms"]') 
            if bedrooms_element is not None : bedrooms = bedrooms_element.text_content() ; bedrooms = None

            type_element = page.query_selector('xpath=//span[@data-aut-id="value_type"]')
            if type_element is not None : property_type = type_element.text_content() ; property_type = None

            bread_element = page.query_selector('xpath=//div[@data-aut-id="breadcrumb"]')
            if bread_element is not None : breadcrumbs = bread_element.text_content() ; breadcrumbs = None

            loc_element = page.query_selector('xpath=//div[@data-aut-id="itemLocation"]')
            if loc_element is not None : location = loc_element.text_content() ; location = None

            sname_element = page.query_selector('xpath=//div[@data-aut-id="profileCard"]')
            if sname_element is not None : seller_name = sname_element.text_content() ; seller_name = None
            ...
            json_data = json.loads(json_util.dumps(property_data))
